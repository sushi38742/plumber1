...
export default function ServiceArea() {
  const ref = useFadeIn()
  return (
    <section className="bg-[#F0F9FF] py-20 md:py-28 border-t border-sky-100">
      <div className="max-w-6xl mx-auto px-6">
        <div ref={ref} className="fade-section">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr] gap-12 md:gap-24 mt-8">
            <div>
              <h2 className="text-4xl md:text-5xl font-black text-gray-900 tracking-tight leading-none mb-6">Proudly Serving<br />South Florida.</h2>
            ...
            <div className="flex flex-wrap content-start gap-2">
              {cities.map((city) => (
                <span key={city} className="text-[12px] font-bold text-gray-600 bg-white border border-gray-200 px-3.5 py-2 tracking-wide">{city}</span>
              ))}
              <span className="text-[12px] font-bold text-[#0EA5E9] border border-[#0EA5E9]/30 bg-sky-50 px-3.5 py-2 tracking-wide">+ surrounding areas</span>
            </div>
          </div>
        </div>
      </div>
