...
function ReasonItem({ reason: r, index: i }) {
  const ref = useFadeIn()
  return (
    <div ref={ref} className="fade-section py-12 lg:py-0 lg:px-10 first:pl-0 last:pr-0 border-b lg:border-b-0 lg:border-l border-gray-200 first:border-l-0" style={{ transitionDelay: `${i * 100}ms` }}>
      <div className="w-8 h-[3px] bg-[#0EA5E9] mb-6" />
      <div className="flex items-baseline gap-1 mb-5">
        <span className="text-[4.5rem] md:text-[5rem] font-black text-gray-900 leading-none tabular-nums tracking-tighter">{r.stat}</span>
        <span className="text-2xl font-black text-[#0EA5E9] leading-none">{r.unit}</span>
      </div>
    ...
    <section id="why-us" className="bg-white py-20 md:py-28 overflow-hidden">
      <div className="max-w-6xl mx-auto px-6">
        <div ref={titleRef} className="fade-section flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          ...
          <p className="text-gray-400 text-sm max-w-xs leading-relaxed">Every number here reflects a real commitment — not a marketing claim.</p>
        </div>
